    ...
    while not valid:
        ...
        # Replace a random character
            ...
            if random_index == 0:
                ...
                selfie_mutated_chars = chars_selfie[:random_index] + [random_character] + chars_selfie[
                                                                                          random_index + 1:]
...
def stoned_run(ctx, task, i):

    fp_type = ctx['main_config']['stoned_fp_type']
    collection_file = task['collection_file_tmp']

    with open(collection_file, 'r') as f:
        input_smis = f.read().splitlines()

    # If we still need to sample but have no ligands to mutate left, we start from the first again
    if i >= len(input_smis):
        i = 0
    ...
    num_random_samples = int(ctx['main_config']['sample_interval_size'])
    num_mutation_ls = list(range(1, int(ctx['main_config']['stoned_num_mutations']) + 1))

    mol = Chem.MolFromSmiles(input_smis[i])
    if mol == None:
    ...
    start_time = time.time()
    canon_smi_ls_scores = get_fp_scores(canon_smi_ls, target_smi=input_smis[i], fp_type=fp_type)
    print('Fingerprint calculation time: ', time.time() - start_time)
    print('Total time: ', time.time() - total_time)

    # Select molecules with certain fingerprint similarity
    indices_thresh = [i for i, x in enumerate(canon_smi_ls_scores) if
                      float(ctx['main_config']['stoned_fp_similarity_min']) < x < float(
                          ctx['main_config']['stoned_fp_similarity_max'])]
    canon_smis = [canon_smi_ls[idx] for idx in indices_thresh]
